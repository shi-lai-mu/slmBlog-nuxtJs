import { BackgroundSpace } from '@/interface/utiles/background'
...
export default class Background {
  ...
  constructor(canvas: HTMLCanvasElement, options?: BackgroundSpace.Options, auto = true) {
    ...
    this.options = options
      ? Object.assign({}, Background.defaultOptions, options)
  ...
  private draw() {
    ...
    this.anObject.grains.forEach((grain, i) => {
      ...
      if (grain.accelerate > 0.5) grain.accelerate -= 0.001
      ...
      // 超出界限重生
      if (grain.y < -100 || grain.x < -100 || grain.y > height + 100 || grain.x > width + 100) {
        this.anObject.grains[i] = this.createGrain()
      }
