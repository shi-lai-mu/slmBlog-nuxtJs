...
export class BaseValidateData {
  ...
  validate?(data: Object) {
    ...
    const success = this._echo(data, (value, key) => {
      ...
      } else if ($EntityOptions.isFull)
        errorArray.push(ValidateError.toStatus(key, value, 'ExtraField'))
   ...
   * @param cb  回调
   */
  private _echo?(obj: Object, cb: CallableFunction) {
...
export class ValidateError {
   ...
   * 输出状态
   */
  static readonly toStatus = (
    key,
    value,
    msg: string,
    params?: unknown[]
  ): ValidateNS.ErrorState => ({
    status: false,
...
export class ValidateRules {
  static lengthValidate(value: string, options: ColumnOptions, length: string | number) {
