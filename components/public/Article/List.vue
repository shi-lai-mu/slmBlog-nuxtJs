<template>
  <div class="article-list-box">
    <aside v-if="!$store.state.isMobile" class="layout-aside">
      <div v-for="(item, index) of getListAsideConfig.list" :key="index" class="aside-child-box">
        <tooltip v-for="(chiItem, chiIndex) of item" :key="chiIndex" placement="left">
    ...
    <div
      ...
      :class="['article-list-item', layout.name, { 'is-open': viewId === item.id }]"
    >
      <ArticleView
        :key="index"
        :ssr="item"
        :view-id="viewId"
        @open="openArticleEvent"
        @close="closeArticleEvent"
      />
    </div>
  </div>
</template>
...
$layoutTransition: 0.5s ease-in-out;
...
.article-list-box {
  ...
  .layout-aside {
    ...
    flex-wrap: wrap;
    transition: $layoutTransition 0.5s;
  ...
  /deep/ .article-list-item {
    ...
    &:hover {
      .figure-cover {
        box-shadow: 0 0 10px rgba($color: #000, $alpha: 0.9) inset;
