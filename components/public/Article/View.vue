  ...
  <div class="article-card">
    ...
    >
      <template v-if="!isOpen">
        ...
        />
        <i v-else class="slm blog-img-err figure-cover"></i>
        <div class="article-content">
          ...
          ></router-link>
          <p
            class="line-ellipsis double-line-ellipsis article-description"
      ...
      <ArticleContent v-if="isOpen" :ssr="article" init-skeleton />
    </div>
...
export default class ArticleView extends Vue {
  ...
  openView(articleId: number) {
    ...
    const parentEl = el.children[0].parentElement
  ...
  closeView() {
    ...
    if (task.length) task.forEach(v => clearTimeout(v as number))
      ...
      setTimeout(() => {
        ...
        task.push(
          setTimeout(() => {
            this.style = {}
            this.task = []
            ;(el.children[0].parentElement as Element).setAttribute('style', '')
          }, 600)
        )
      })
    )
