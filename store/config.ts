  ...
  mutations: {
    ...
    setWebOptions(state, payload) {
      ...
      if (isSave) {
        ...
        const config = GlobalTool.excludeKey(defaultsDeep({}, state.setting), [
          'title',
          'description',
          'type',
          'config',
        ...
        saveUserConfigClock = setTimeout(async () => {
          await saveUserConfig(config as unknown as User.Config)
      ...
      state.setting.config = {
        ...
        } as unknown as typeof state['setting']['config']['theme'],
      }
    },
  ...
  actions: {
    ...
    saveConfigServer(store, payload) {
      ...
      if (isSave) {
        ...
        delete config.config
        $cookie.set('web', JSON.stringify(config), { expires: 365 })
        if ((rootState as unknown as { [k: string]: boolean }).jwt) {
          clearTimeout(saveUserConfigClock)
          saveUserConfigClock = setTimeout(async () => {
            await saveUserConfig(config as unknown as User.Config)
          }, 1000)
